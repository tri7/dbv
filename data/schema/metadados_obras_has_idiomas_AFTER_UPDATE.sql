CREATE DEFINER=`root`@`localhost` TRIGGER `cpm`.`metadados_obras_has_idiomas_AFTER_UPDATE`
AFTER UPDATE ON `cpm`.`metadados_obras_has_idiomas`
FOR EACH ROW
begin
...
set wservice_terms_out := (select status from servicosweb where name = 'terms_out');
	if @service_meta_idiomas = 0 and wservice_terms_out = 1 then
        ...
        set idiomas := (select group_concat(distinct i.idioma order by i.idioma asc separator ';') 
            ...
            where mdohi.metadados_obras_id = new.metadados_obras_id 
		...
		insert into wsaux set row_id = new_fieldsid, table_name = 'metadados_obras_has_idiomas', 
			oldfields = ifnull(@idiomas_old,''), newfields = new_fields, termsid = productId, method = 'PUT', trigger_com = 'UPDATE';
	end if;
end