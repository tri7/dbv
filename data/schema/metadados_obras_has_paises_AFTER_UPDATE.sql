CREATE DEFINER=`root`@`localhost` TRIGGER `cpm`.`metadados_obras_has_paises_AFTER_UPDATE`
AFTER UPDATE ON `cpm`.`metadados_obras_has_paises`
FOR EACH ROW
begin
...
set wservice_terms_out := (select status from servicosweb where name = 'terms_out');
	...
	if @service_meta_paises = 0 and wservice_terms_out = 1 then
        ...
        set paises := (select group_concat(distinct p.pais order by p.pais asc separator ';') 
			...
			mdohp.paises_pais_id = p.pais_id where mdohp.metadados_obras_id = new.metadados_obras_id 
            group by mdohp.metadados_obras_id);

        set new_fields := concat_ws(' ### ',new.metadados_obras_id, ifnull(paises,''));
        set new_fieldsid := concat_ws(' ### ',new.metadados_obras_id, new.paises_pais_id);
            
		insert into wsaux set row_id = new_fieldsid, table_name = 'metadados_obras_has_paises', 
			oldfields = ifnull(@paises_old,''), newfields = ifnull(new_fields,''), termsid = productId, method = 'PUT', trigger_com = 'UPDATE';
	end if;
end