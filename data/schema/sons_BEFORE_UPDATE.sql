CREATE DEFINER=`root`@`localhost` TRIGGER `cpm`.`sons_BEFORE_UPDATE`
BEFORE UPDATE ON `cpm`.`sons`
FOR EACH ROW
begin
...
declare tmix tinyint(1) default 0;
	...
	# tipo_de_mix
    set tmix := (select (new.tipo_de_mix is not null and old.tipo_de_mix is null) 
		or (new.tipo_de_mix is null and old.tipo_de_mix is not null)
		or (new.tipo_de_mix is not null and old.tipo_de_mix is not null and old.tipo_de_mix != new.tipo_de_mix));
