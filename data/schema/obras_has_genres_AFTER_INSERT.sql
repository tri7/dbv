CREATE DEFINER=`root`@`localhost` TRIGGER `cpm`.`obras_has_genres_AFTER_INSERT`
AFTER INSERT ON `cpm`.`obras_has_genres`
FOR EACH ROW
begin
...
set wservice_terms_out := (select status from servicosweb where name = 'terms_out');
    ...
    if @service_ohgenres = 0 and wservice_terms_out = 1 then
		...
		insert into wsaux set row_id = new_fieldsid, table_name = 'obras_has_genres', 
			oldfields = ifnull(@genero_old,''), newfields = ifnull(new_fields,''), termsid = productId, method = 'PUT', trigger_com = 'INSERT';
	end if;
end