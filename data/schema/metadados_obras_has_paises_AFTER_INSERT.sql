CREATE DEFINER=`root`@`localhost` TRIGGER `cpm`.`metadados_obras_has_paises_AFTER_INSERT`
AFTER INSERT ON `cpm`.`metadados_obras_has_paises`
FOR EACH ROW
begin
...
set wservice_terms_out := (select status from servicosweb where name = 'terms_out');
	if @service_meta_paises = 0 and wservice_terms_out = 1 then
		...
		insert into wsaux set row_id = new_fieldsid, table_name = 'metadados_obras_has_paises', 
			oldfields = ifnull(@paises_old,''), newfields = ifnull(new_fields,''), termsid = productId, method = 'PUT', trigger_com = 'INSERT';
	end if;
end