CREATE DEFINER=`root`@`localhost` TRIGGER `cpm`.`metadados_obras_has_synopsis_AFTER_DELETE`
AFTER DELETE ON `cpm`.`metadados_obras_has_synopsis`
FOR EACH ROW
begin
...
set wservice_terms_out := (select status from servicosweb where name = 'terms_out');
	if /*@service_synopsis = 0 and*/ wservice_terms_out = 1 then
		...
		insert into wsaux set row_id = old_fields, table_name = 'metadados_obras_has_synopsis', 
